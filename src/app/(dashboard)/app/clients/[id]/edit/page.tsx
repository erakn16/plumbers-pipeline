...
import { useCRMStore, Client } from "@/store/crm-store";
...
export default function EditClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { getClientById, updateClient } = useCRMStore();
  ...
  const [addresses, setAddresses] = useState(
    client?.addresses?.length ? client.addresses : [
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addToast(`${updates.name} updated successfully.`, "success");
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <button onClick={() => router.push(`/app/clients/${client.id}`)} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
        <ArrowLeft className="w-4 h-4" /> Back to Client
      </button>

      <div>
        <h1 className="text-2xl font-bold tracking-tight text-foreground">Edit Client</h1>
        <p className="text-sm text-muted-foreground mt-1">Update details for {client.name}.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <FormField label="Company">
            <input type="text" value={form.company} onChange={(e) => updateField("company", e.target.value)} className="form-input" />
          </FormField>
        ...
        <div className="border border-border rounded-xl p-4 bg-card/50 space-y-4">
          <div className="flex items-center justify-between border-b border-border pb-3">
             ...
             <button type="button" onClick={addAddress} className="text-xs font-medium text-primary hover:underline inline-flex items-center gap-1"><Plus className="w-3.5 h-3.5" /> Add Address</button>
          </div>
          ...
          {addresses.map((addr, idx) => (
            <div key={addr.id} className="p-3 border border-border rounded-lg bg-background relative space-y-3">
              <div className="flex justify-between items-center mb-1">
                ...
                <button type="button" onClick={() => removeAddress(idx)} className="p-1 text-muted-foreground hover:text-destructive"><Trash2 className="w-4 h-4" /></button>
              </div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-border">
          <button type="button" onClick={() => router.push(`/app/clients/${client.id}`)} className="px-4 py-2.5 text-sm font-medium rounded-md border border-border bg-card text-foreground hover:bg-secondary transition-colors">
            Cancel
          </button>
