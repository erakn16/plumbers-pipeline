...
export default function NewWorkerPage() {
  ...
  const { addWorker, addActivity } = useCRMStore();
  ...
  const [form, setForm] = useState({
    ...
    email: "",
    phone: "",
    role: setup.workerRoles[0] || "Technician",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addActivity({
      ...
      description: `New worker added: ${newWorker.name}`,
      entityType: "worker",
      entityId: newWorker.id,
      entityName: newWorker.name,
      userId: "current_user",
      userName: "You",
      timestamp: new Date().toISOString(),
    });
    addToast(`${newWorker.name} added to the team.`, "success");
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-foreground">Add New Worker</h1>
        <p className="text-sm text-muted-foreground mt-1">Add a team member to your workforce.</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField label="Full Name" required helperText="Letters, spaces, hyphens, apostrophes only">
            <input type="text" value={form.name} onChange={(e) => updateField("name", sanitizeName(e.target.value))} required className="form-input" placeholder="Mike DiNapoli" />
          </FormField>
          <FormField label="Role">
            <select value={form.role} onChange={(e) => updateField("role", e.target.value)} className="form-input">
              {setup.workerRoles.map((role) => (
                <option key={role} value={role}>{role}</option>
              ))}
              {setup.workerRoles.length === 0 && <option value="Technician">Technician</option>}
            </select>
          </FormField>
          <FormField label="Email" required>
            <input type="email" value={form.email} onChange={(e) => updateField("email", e.target.value)} required className="form-input" placeholder="mike@plumberspipeline.com" />
          </FormField>
          <FormField label="Phone">
            <input type="tel" value={form.phone} onChange={(e) => updateField("phone", e.target.value)} className="form-input" placeholder="(404) 555-1234" />
          </FormField>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <FormField label="Hourly Rate ($)">
            <input type="number" step="0.01" min="0" value={form.hourlyRate} onChange={(e) => updateField("hourlyRate", e.target.value)} className="form-input" placeholder="45.00" />
          </FormField>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-border">
          <button type="button" onClick={() => router.push("/app/workers")} className="px-4 py-2.5 text-sm font-medium rounded-md border border-border bg-card text-foreground hover:bg-secondary transition-colors">
            Cancel
          </button>
