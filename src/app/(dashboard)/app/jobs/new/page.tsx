...
import { ArrowLeft, Plus, Trash2, Tag, MessageSquareText, Upload, X } from "lucide-react";
...
export default function NewJobPage() {
  ...
  const { addJob, addActivity, updateEstimate, clients, workers, estimates } = useCRMStore();
  const { setup, getNextNumber } = useSettingsStore();
  const { addToast } = useToast();

  const fromEstimateId = searchParams.get("fromEstimate");
  const sourceEstimate = fromEstimateId ? estimates.find((e) => e.id === fromEstimateId) : null;

  const [form, setForm] = useState({
    title: "",
    clientId: sourceEstimate?.clientId || "",
    customerPO: sourceEstimate?.customerPO || "",
    status: "open" as "open" | "scheduled" | "in-progress" | "completed" | "on-hold" | "cancelled" | "invoice_created",
    ...
    description: "",
    serviceAddressId: "",
    serviceAddressCustom: sourceEstimate?.serviceAddress || "",
    notes: "",
    estimateId: sourceEstimate?.id || "",
  });

  const [lineItems, setLineItems] = useState<EstimateLineItem[]>(
    sourceEstimate?.lineItems?.length
      ? sourceEstimate.lineItems.map((li) => ({ ...li }))
  ...
  const [discountMode, setDiscountMode] = useState<"none" | "percent" | "fixed">(
    sourceEstimate?.discount ? "fixed" : "none"
  );
  const [activePresetId, setActivePresetId] = useState<string | null>(null);
  const [activePresetPercent, setActivePresetPercent] = useState(0);
  const [fixedDiscount, setFixedDiscount] = useState(sourceEstimate?.discount?.toString() || "");
  ...
  // If from estimate, set serviceAddressCustom instead of dropdown
  useEffect(() => {
    if (sourceEstimate?.serviceAddress) {
      setForm(p => ({ ...p, serviceAddressId: "__custom__", serviceAddressCustom: sourceEstimate.serviceAddress || "" }));
    }
  }, [sourceEstimate]);
  ...
  // Append default job note to description on mount
  useEffect(() => {
    if (setup.defaultJobNote) {
      setForm((p) => ({
        ...p,
        description: p.description || `\n\n---\n${setup.defaultJobNote}`,
      }));
    }
  }, [setup.defaultJobNote]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (sourceEstimate) {
      updateEstimate(sourceEstimate.id, { status: "job_created", jobId: newJob.id });
    }

    addActivity({
      id: "act_" + Date.now().toString(36),
      type: "job_created",
      description: `Job ${jobNumber} created: ${form.title}`,
      entityType: "job",
      entityId: newJob.id,
    ...
    addToast(`Job ${jobNumber} created successfully.`, "success");
  ...
  const toggleWorker = (wId: string) => {
    setForm((prev) => ({
      ...
      assignedWorkerIds: prev.assignedWorkerIds.includes(wId)
        ? prev.assignedWorkerIds.filter((id) => id !== wId)
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        {sourceEstimate && <p className="text-sm text-muted-foreground mt-1">Converting from estimate {sourceEstimate.estimateNumber}</p>}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 border border-border bg-card p-5 rounded-xl">
          <FormField label="Job Title" required>
            <input type="text" value={form.title} onChange={(e) => updateField("title", e.target.value)} required className="form-input" placeholder="Water Heater Replacement" />
          </FormField>
          ...
          <FormField label="Customer PO #">
            <input type="text" value={form.customerPO} onChange={(e) => updateField("customerPO", e.target.value as string)} className="form-input" placeholder="PO-10293" />
          </FormField>

          <FormField label="Service Address">
            {clientAddresses.length > 0 ? (
              <select value={form.serviceAddressId} onChange={(e) => updateField("serviceAddressId", e.target.value as string)} className="form-input">
              ...
              <input type="text" value={form.serviceAddressCustom} onChange={(e) => { updateField("serviceAddressCustom", e.target.value as string); updateField("serviceAddressId", "__custom__"); }} className="form-input" placeholder="123 Main St..." />
            )}
            {form.serviceAddressId === "__custom__" && (
              <input type="text" value={form.serviceAddressCustom} onChange={(e) => updateField("serviceAddressCustom", e.target.value as string)} className="form-input mt-2" placeholder="Enter custom address..." />
            )}
          </FormField>
              ...
              <tbody className="divide-y divide-border">
                {lineItems.map((item, idx) => (
                  <tr key={idx} className="group hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="p-2 align-top"><input type="number" min="0" step="1" value={item.quantity} onChange={(e) => updateLineItem(idx, "quantity", parseInt(e.target.value) || 0)} className="form-input text-sm text-right font-mono" /></td>
                    <td className="p-2 align-top"><input type="number" min="0" step="1" value={item.unitPrice} onChange={(e) => updateLineItem(idx, "unitPrice", parseInt(e.target.value) || 0)} className="form-input text-sm text-right font-mono" /></td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField label="Description">
            <textarea value={form.description} onChange={(e) => updateField("description", e.target.value)} className="form-input min-h-[100px] resize-y text-sm" />
          </FormField>
          <FormField label="Notes">
            <textarea value={form.notes} onChange={(e) => updateField("notes", e.target.value)} className="form-input min-h-[100px] resize-y text-sm" placeholder="Internal notes..." />
          </FormField>
        </div>
        ...
        <div className="border border-border rounded-xl p-4 bg-card/50 space-y-3">
          ...
          {activeWorkers.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-3">
              {activeWorkers.map((w) => (
                <label key={w.id} className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
                  <input type="checkbox" checked={form.assignedWorkerIds.includes(w.id)} onChange={() => toggleWorker(w.id)} className="accent-primary rounded" />
                  {w.name}
                  <span className="text-[10px] text-muted-foreground bg-secondary px-1.5 py-0.5 rounded">{w.role}</span>
                </label>
              ))}
            </div>
          )}
        ...
        <div className="border border-border rounded-xl p-4 bg-card/50 space-y-3">
          ...
          <label className="inline-flex items-center gap-1.5 text-xs text-primary hover:underline font-medium cursor-pointer">
            ...
            <div className="flex flex-wrap gap-3 mt-2">
              {images.map((img, idx) => (
                <div key={idx} className="relative group w-20 h-20 rounded-lg overflow-hidden border border-border">
                  ...
                  <button type="button" onClick={() => setImages(prev => prev.filter((_, i) => i !== idx))} className="absolute top-0.5 right-0.5 bg-black/60 text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ))}
        ...
        <div className="flex justify-end gap-3 pt-6">
          <button type="button" onClick={() => router.push("/app/jobs")} className="px-5 py-2.5 text-sm font-medium rounded-md border border-border bg-card text-foreground hover:bg-secondary transition-colors">Cancel</button>
          <button type="submit" className="px-8 py-2.5 text-sm font-medium rounded-md bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm transition-colors active:scale-[0.98]">Create Job</button>
        </div>
      </form>
    </div>
  );
